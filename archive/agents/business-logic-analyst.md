...
6. `output/citations/business-logic-analyst-citations.md` - **MANDATORY citations reference file**
...
- **COMPLETE LISTING**: If you find 67 rules, you MUST document ALL 67 rules in the catalog
- **NO SUMMARIZATION**: Do not show "examples" or "key rules" - show EVERY SINGLE rule found
- **CONSISTENT CITATIONS**: Use REF-XXX format consistently across ALL documents
- **MATCHING COUNTS**: The total count MUST match the actual number of rules displayed
...
Write("output/docs/business-logic-analysis.md", documentation)
...
def generate_business_rules_catalog(business_rules):
    """Generate COMPLETE catalog of ALL business rules - NO SUMMARIZATION"""
    catalog = f"""# Business Rules Catalog

**Generated**: {datetime.now().isoformat()}
**Purpose**: COMPLETE documentation of ALL business logic found in codebase
**Total Rules**: {len(business_rules)}
⚠️ **CRITICAL**: This catalog MUST list ALL {len(business_rules)} rules below, not just examples
...
"""
        ...
        for idx, rule in enumerate(rules, 1):
            ...
            ref_id = f"REF-{self.ref_counter:03d}"
            self.ref_counter += 1

            catalog += f"""#### {rule['id']} [{ref_id}]
...
- [ ] **Verified: Consistent REF-XXX numbering used across ALL documents**
- [ ] **Verified: No missing REF numbers (continuous sequence)**
- [ ] **Sequence diagrams created for ALL key business flows**
- [ ] **CRITICAL: Citations file generated at `output/citations/business-logic-analyst-citations.md`**
- [ ] **CRITICAL: ALL Mermaid diagrams validated with zero errors**
- [ ] Agent completion message displayed

### Step 5: Generate Citations File (MANDATORY)
```bash
# Generate agent-specific citations file
AGENT_NAME="business-logic-analyst"
mkdir -p output/citations

# Copy template
cp framework/templates/citations_template.md output/citations/${AGENT_NAME}-citations.md

# Update placeholders
sed -i '' "s/{AGENT_NAME}/${AGENT_NAME}/g" output/citations/${AGENT_NAME}-citations.md
sed -i '' "s/\[TIMESTAMP\]/$(date '+%Y-%m-%d %H:%M:%S')/g" output/citations/${AGENT_NAME}-citations.md

echo "✅ Created citations template at output/citations/${AGENT_NAME}-citations.md"
```

Then manually populate the citations file with actual findings, including:
- All REF-XXX references used in documentation
- Component names and locations
- Which files use each citation (e.g., "Used in: business-rules-catalog.md, business-logic-domain.mmd")
