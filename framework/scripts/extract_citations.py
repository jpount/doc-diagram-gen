...
def assign_ref_ids(components):
    """Assign unique REF-XXX IDs to all components"""
    ref_counter = 0
    export_data = {}
    ref_index = {}  # Maps REF-XXX to component details for quick lookup
    ...
    print(f"✅ Assigned {ref_counter} REF-XXX identifiers")
...
def main(args):
    ...
    # Export to JSON with REF-XXX IDs
    print("\n📝 Assigning REF-XXX identifiers...")
    ...
    export_data["metadata"] = {
        ...
        "version": "1.1",  # Updated version with REF-XXX
...
def validate_extraction(citations_path: str):
    ...
    if 'methods' in data and data['methods']:
        ...
        for method in data['methods'][:3]:
            print(f"  - {method['name']} ({method.get('parent_class', 'global')})")
