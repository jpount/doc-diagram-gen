...
Provides REF-XXX style citations and tracks usage across documents.
"""
...
class CitationManager:
    ...
    def __init__(self, citations_json_path: str = "output/context/codebase-citations.json"):
        ...
        self.ref_counter = 0
        self.ref_mapping: Dict[str, Dict] = {}  # REF-XXX -> component details
        self.usage_tracking: Dict[str, Set[str]] = {}  # REF-XXX -> set of files using it
    ...
    def load_citations(self) -> bool:
        ...
        # Load the pre-assigned REF-XXX mappings
    ...
    def lookup_citation(self, name: str, component_type: Optional[str] = None) -> Optional[str]:
        """
        Simple lookup - returns just the REF-XXX for a component.
    ...
    def get_citation_details(self, ref_id: str) -> Optional[Dict]:
        """Get full details for a REF-XXX citation"""
...
- **Context**: {snippet[:100]}{'...' if len(snippet) > 100 else ''}
- **Used in**: {used_in_str}
...
"""
    ...
    def update_document_with_references(self, content: str, doc_name: str) -> Tuple[str, int]:
        """
        Update a document to use REF-XXX format for any inline citations.
    ...
    def validate_citations_in_document(self, file_path: str) -> Dict:
        """Validate that all REF-XXX in a document are valid"""
        path = Path(file_path)
        if not path.exists():
        ...
        # Find all REF-XXX patterns
        refs = re.findall(r'REF-\d{3}', content)
