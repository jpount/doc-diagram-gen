...
### 1. REF-XXX: Code Location References (ALL AGENTS)
- **Format**: `REF-001` onwards
- **Usage**: Reference specific code locations, files, classes, methods
- **Example**: "The `UserService` class [REF-001] handles authentication"
- **Required by**: ALL agents

### 2. Agent-Specific Citations

#### Business Logic Analyst
- **BR-XXX**: Deterministic business rules (e.g., BR-001 to BR-039)
- **BR-LLM-XXX**: LLM-discovered business rules with confidence levels
...
**In Documentation**: `ComponentName` [REF-XXX]
- Example: "The `UserService` class [REF-001] handles authentication"
...
classDiagram
    %% Component Citations
    %% UserService: REF-001 (src/main/java/UserService.java:45)
    %% OrderRepository: REF-002 (src/main/java/OrderRepository.java:23)
```
...
- Use [REF-XXX] format in all documentation
...
sequenceDiagram
    ...
    %% UserService: REF-001
    %% OrderController: REF-002

    participant UserService
    participant OrderController
```
**Note**: Keep diagram citations clean (just REF-XXX) for better readability
...
# Check documentation uses [REF-XXX] format
if ! grep -q "\[REF-" output/docs/*.md 2>/dev/null; then
    echo "❌ WARNING: No [REF-XXX] citations found in documentation"
fi
