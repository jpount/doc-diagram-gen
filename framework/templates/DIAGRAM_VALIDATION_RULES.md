...
**EVERY diagram MUST include citation comments immediately after the diagram type declaration.**
...
classDiagram
    ...
    %% TradeDirect: REF-001 (line 698)
    %% TradeServices interface: REF-002 (line 8565)
    %% OrderDataBean: REF-003 (line 6115)
...
sequenceDiagram
    ...
    %% TradeDirect.login: REF-004 (line 2341)
...
flowchart TD
    ...
    %% TradeDirect.buy(): REF-010 (line 834)
    %% Balance check: REF-011 (line 856)
    %% Quote validation: REF-012 (line 848)
...
stateDiagram-v2
    ...
    %% OrderDataBean.orderStatus: REF-015 (line 6136)
    %% Status values: REF-016 (lines 6140-6145)
    %% isOpen() method: REF-017 (line 6384)
...
erDiagram
    ...
    %% AccountDataBean: REF-017 (@Entity at line 4919)
    %% OrderDataBean: REF-018 (@Entity at line 6115)
    %% HoldingDataBean: REF-019 (@Entity at line 5412)
    %% QuoteDataBean: REF-020 (@Entity at line 6486)
...
gantt
    %% Component Citations
    %% ProjectPhases: REF-021 (docs/project-plan.md:45)
    %% Milestones: REF-022 (src/config/project-config.json:12)
...
graph LR
    %% Component Citations
    %% Gateway: REF-023 (src/gateway/ApiGateway.java:15)
...
### Clean Diagram Citations
**In diagrams: Keep citations minimal - just REF-XXX**
```
%% Component Citations
%% ComponentName: REF-XXX
```

**In citations.md: Full details**
```markdown
### REF-XXX: ComponentName
- Original file: `path/to/file.ext:lineNumber`
- Repomix line: 2456
- Context: Main authentication service
- Code snippet: [included]
```

### Special Cases in Diagrams
- **External Systems**: `%% PaymentGateway: External`
- **Configuration**: `%% Database: REF-XXX`
- **Not Found**: `%% CacheLayer: NOT FOUND`
- **Multiple Files**: `%% Controllers: REF-XXX`

### Example Flow
1. **In Diagram** (clean and simple):
   ```mermaid
   classDiagram
       %% Component Citations
       %% UserService: REF-001
       %% AuthRepository: REF-002
   ```

2. **In citations.md** (full details):
   ```markdown
   ### REF-001: UserService
   - Original file: `src/services/UserService.java:45`
   - Repomix line: 2456
   - Context: Handles user authentication and session management

   ### REF-002: AuthRepository
   - Original file: `src/repos/AuthRepository.java:23`
   - Repomix line: 3421
   - Context: Database access for authentication data
   ```
